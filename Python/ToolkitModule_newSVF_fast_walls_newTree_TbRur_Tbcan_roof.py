...
def modelRun(nt, cfM, lc_data, met_data_all, date1A, Dats):
    ...
    tmstp = cfM['timestep']+ 'D'                              # time step (minutes)
...
#                met_data_all.ix[i]['Ld'] = ld_mod(met_data_all.ix[i])['Ld_md'] ## Ld_mod is added to meteorological forcing data frame
        ...
        #mod_rslts=np.zeros((nt-8000,len(lc_data),1),  np.dtype([('ID',np.int32),('Ws', '<f8'), ('Ts','<f8'), ('Ta','<f8'),('date',object)]))
        #
        ...
        #mod_rslts=np.zeros((nt,len(lc_data),1),  np.dtype([('ID',np.int32),('Ws', '<f8'), ('Ts','<f8'), ('Ta','<f8'),('Rn', '<f8'),('Qg', '<f8'), ('Qe','<f8'), ('Qh','<f8'),('date',object)])) 
        ...
        for i in range(0, len(met_data_all)):
            mod_rslts=np.zeros((1,len(lc_data),1),  np.dtype([('ID',np.int32),('Ws', '<f8'), ('Ts','<f8'), ('Ta','<f8'),('date',object)]))
            mod_rslts_tmrt_utci=np.zeros((1,len(lc_data),1),  np.dtype([('ID',np.int32),('tmrt', '<f8'), ('utci','<f8'),('date',object)]))
            print ('Mmmbop')
            print ( len(met_data_all) ) 
            if not i == len(met_data_all)-1: 
                ...
                for grid in range(0,len(lc_data)):      # now cycle through each grid point
                    ...
                    if not i == 0:
                        mod_data_ts_[i][9]['Veg'] = mod_rslts_prev[0][grid]['Ta']
                    else:
                    ...
                    if not i == 0:
                        Tacprv=mod_rslts_prev[0][grid]['Ta']
                    else:
                        Tacprv = met_d['Ta'][i]
                ...
                # Lc = LC_wRoofAvg
                    ...
                    tmrt = getTmrtForGrid_RH(wS_Ta['Ta_f'],met_d['RH'][i],Ucan,met_d['Kd'][i],tS_can,met_d['Ld'][i], cs.cs['sb']*(met_d['Ta'][i]  +273.15)**4 ,yd_actual, TM, lat)
                    utci = getUTCIForGrid_RH(wS_Ta['Ta_f'],Ucan,met_d['RH'][i],tmrt)
                    ...
                    mod_rslts_tmrt_utci[0][grid] = for_tab_tmrt_utci
            dateString =str(dte)
