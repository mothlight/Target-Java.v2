    ...
    for i in range(0, len(met_data_all)):
        if not i == len(met_data_all)-1: 
            ...
            for grid in range(0,len(lc_data)):      # now cycle through each grid point
                ...
                tS =sum([a*b for a,b in zip(LC2,[mod_data_ts_['roof'][i][9],mod_data_ts_['road'][i][fg],mod_data_ts_['watr'][i][fg],mod_data_ts_['conc'][i][fg],mod_data_ts_['Veg'][i][9],mod_data_ts_['dry'][i][fg],mod_data_ts_['irr'][i][fg],mod_data_ts_['wall'][i][fw]])])    # surface averaged Tsurf
