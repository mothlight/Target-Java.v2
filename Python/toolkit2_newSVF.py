    ...
    for i in range(0, len(met_data_all)):
        if not i == len(met_data_all)-1: 
            ...
            for grid in range(0,len(lc_data)):      # now cycle through each grid point
                ...
                H  = float(lc_data['H'][grid])         # building height for grid point
                W  = float(lc_data['W'][grid])         # stree width for grid point
                if W < 5.0:
                    W = 5.0
                ...
                #################### radiation balance non-water ###########################
                    if not surf in ['watr','Veg']:
                        rad  = rn_calc_new(cs,cfM,met_d,surf,Dats,mod_data_ts_,i,svfg)  # creates dictionary with radiation variables for current timestep and surface type                             
                    ...
                    if (surf == 'watr'):
                        ...
                        mod_data_ts_[i]['watr'] = wtr_stf['TsW']
                        mod_data_tm_[i]['watr'] = wtr_stf['TM']                 
                        mod_data_qh_[i]['watr']= wtr_stf['QhW']
                        mod_data_qe_[i]['watr'] = wtr_stf['QeW']
                        mod_data_qg_[i]['watr']= wtr_stf['QgW']
                        mod_data_ts_[i]['TSOIL'] = wtr_stf['TSOIL']
                        mod_data_rn_[i]['watr'] = rad['Rn']   
                    if (surf == 'Veg'):
                        rad  = rn_calc_new(cs,cfM,met_d,surf,Dats,mod_data_ts_,i,svfg)  # creates dictionary with radiation variables for current timestep and surface type  
                        ##################### ENG BALANCE tree #######################
                        eng_bals=LUMPS(rad,cs,cfM,met_d,surf,Dats,i)            # creates dictionary with energy balance for current timestep and surface type
