...
def Ts_EB_W(met_d,cs,cfM,mod_ts, mod_tm, Dats,i,rad):
    ...
    if Dats['dte'] <= Dats['date1A'] +  timedelta(seconds=(2*int(cfM['timestep'][:-1]))):
        ...
        Gwpls   = -cs.cs['C']['watr'] * cs.cs['Kw'] * ((mod_ts['TSOIL'][i-1]-mod_ts['watr'][i-1])/cs.cs['zW']) + ((met_d['Kd'][i]*(1-0.08))-Sab)					# the convective heat flux at the bottom of the water layer (and into the soil below)
        
        dlt_soil = ((2/(cs.cs['C']['soilW']*cs.cs['dW'])*Gwpls))-(cs.cs['ww']*(mod_ts['TSOIL'][i-1]-mod_tm['watr'][i-1]))				# force restore calc -- change soil temperature change 
        Tsoil = mod_ts['TSOIL'][i-1]+(dlt_soil*int(cfM['timestep'][:-1]))											# soil layer temperature (C)
        ...
        dlt_Tw = Q1/(cs.cs['C']['watr']*cs.cs['zW'])*int(cfM['timestep'][:-1]) 										#  change in surface water Temperature (C)
        ...
        delta_Tm = Gwpls/(cs.cs['C']['soilW']*Dy)		## change in Tm per second
        tM = mod_tm['watr'][i-1] + (delta_Tm*int(cfM['timestep'][:-1]))
